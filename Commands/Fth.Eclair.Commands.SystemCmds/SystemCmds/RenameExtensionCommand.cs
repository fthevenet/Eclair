//----------------------------------------------------------------------------- 
// <copyright file=RenameExtensionCommand">
...
namespace Eclair.Commands.SystemCmds
{
      [CommandInfo(
        ...
        "",
        true)]
    public class RenameExtensionCommand:CommandBase          
    {
        ...
        public override void ExecuteCommand(CommandContext c)
        {
            ...
            string targetExt = c.CommandLineArguments[1];
            string[] files =  Directory.GetFiles(dirName, filter,so);
            for (int i=0; i<files.Length;i++)
            {
                File.Move(files[i], string.Format("{0}\\{1}.{2}", dirName, Path.GetFileNameWithoutExtension(files[i]), targetExt));


            }
            this.OutputInfo("{0} file(s) renamed", files.Length);
